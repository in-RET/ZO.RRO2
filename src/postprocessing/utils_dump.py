...
def calc_energyexport_cost(year,import_price,cleaned_sequences_bus):
    ...
    -------
    export_costs : dict
        export cost for every energy carrier
...
def summarize_sequences_by_name(cleaned_sequences_bus, cleaned_sequences_component):
    """
    Remove suffixes and summarize sequences by component and bus names
    
    Parameters:
    -----------
    cleaned_sequences_bus : dict
        Dict with scenarios -> buses -> components -> sequences
    cleaned_sequences_component : dict
        Dict with scenarios -> components -> buses -> sequences
    
    Returns:
    --------
    dict: Summarized sequences for bus flows (scenario -> bus -> component -> sequence)
    dict: Summarized sequences for component flows (scenario -> component -> bus -> sequence)
    """
    
    def remove_suffix(name):
        suffixes = ['_n', '_e', '_m', '_s', '_north', '_east', '_middle', '_south']
    ...
    summarized_bus_sequences = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
    summarized_component_sequences = defaultdict(lambda: defaultdict(lambda: defaultdict(float)))
    
    print("Processing bus sequences...")
    for scenario, buses in cleaned_sequences_bus.items():
        for bus_with_suffix, components in buses.items():
            base_bus = remove_suffix(bus_with_suffix)
            
            for component_with_suffix, sequence_data in components.items():
                base_component = remove_suffix(component_with_suffix)
                
                if base_component not in summarized_bus_sequences[scenario][base_bus]:
                    if hasattr(sequence_data, 'copy'):
                        summarized_bus_sequences[scenario][base_bus][base_component] = sequence_data.copy()
                    else:
                        summarized_bus_sequences[scenario][base_bus][base_component] = sequence_data
                else:
                    # Add the sequences together
                    summarized_bus_sequences[scenario][base_bus][base_component] += sequence_data
    
    print("Processing component sequences...")
    for scenario, components in cleaned_sequences_component.items():
        for component_with_suffix, buses in components.items():
            base_component = remove_suffix(component_with_suffix)
            
            for bus_with_suffix, sequence_data in buses.items():
                base_bus = remove_suffix(bus_with_suffix)
                
                if base_bus not in summarized_component_sequences[scenario][base_component]:
                    if hasattr(sequence_data, 'copy'):
                        summarized_component_sequences[scenario][base_component][base_bus] = sequence_data.copy()
                    else:
                        summarized_component_sequences[scenario][base_component][base_bus] = sequence_data
                else:
                    # Add the sequences together
                    summarized_component_sequences[scenario][base_component][base_bus] += sequence_data
    
    return dict(summarized_bus_sequences), dict(summarized_component_sequences)
    

def prepare_two_day_data(sequences_dict, days, hours_per_day=24):
    """
    Prepare data for two-day plotting by summing values
    
    """
    
    two_day_data = {}
    
    for key1, middle_dict in sequences_dict.items():
        two_day_data[key1] = {}
        for key2, inner_dict in middle_dict.items():
            two_day_data[key1][key2] = {}
            for key3, sequence in inner_dict.items():
                if hasattr(sequence, 'values'):
                    values = sequence.values
                else:
                    values = np.array(sequence)
                total_hours = days * 2 * hours_per_day
                if len(values) < total_hours:
                    values = np.pad(values, (0, total_hours - len(values)), 'constant')
                else:
                    values = values[:total_hours]
                
                # Reshape to daily blocks and sum over 2-day periods
                daily_values = values.reshape(-1, hours_per_day)
                two_day_values = np.sum(daily_values[:days*2].reshape(-1, hours_per_day*2), axis=1)
                
                two_day_data[key1][key2][key3] = two_day_values
