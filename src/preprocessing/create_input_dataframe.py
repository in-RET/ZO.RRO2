...
def readInFiles(folder) -> dict:
    ...
    for f in listdir(folder):
        if isfile(join(folder, f)) and f.endswith('.csv'):
            ...
            sheetdata = pd.read_csv(join(folder, f), delimiter=';', decimal=',', index_col='index', encoding='unicode_escape')
...
def createDataFrames() -> (dict, dict):
    dataRoot = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'preprocessing'))
