        ...
        for r in region: 
            ...
            if s == 'electricity':
                ...
                demand_profile_dict[s][r] = ((load_profile_nom['Base_demand_profile']['base_load']* (float(scalars['Demand_Transport_endenergie_'+ r + '_b']['Gueterverkehr_'+str(YEAR)]['Summe'])*(float(scalars['Demand_Transport_endenergie_'+r+'_b']['Gueterverkehr_'+str(YEAR)]['PKW - Wasserstoff']))))+
                                             ...
                                             (load_profile_nom['other_demand_profile']['Prozessgas'] *(float(scalars['Demand_Industry_' + r + '_b']['Prozesswaerme_'+str(YEAR)]['Summe']) * (float(scalars['Demand_Industry_'+ r + '_b']['Prozesswaerme_' + str(YEAR)]['Heizkessel Wasserstoff'])/100) /float(scalars['Demand_Industry_'+ r + '_b']['EER_' + str(YEAR)]['Heizkessel Wasserstoff'])))
                                             )*1000000
            elif s== 'fuel':
...
def create_user_level_demands(scalars, sequences, YEAR, region=True):
    ...
    demand_profile_dict = {}
    for d in demands:
        demand_profile_dict[d] = {}
        for r in regions:
#------------------------------------------------------- ###### Haushalte #######----------------------------------------------------------------------------
            # -------------------------
            # Space heating (household)
            # -------------------------
            if d == 'space_heating_household':
                demand_profile_dict[d][r] = (
...
#------------------------------------------------------- ###### Industry #######----------------------------------------------------------------------------
            ...
            # -------------------------
            elif d == 'process_heating_industry':
                demand_profile_dict[d][r] = (
            ...
            elif d == 'material_usage_industry':
                demand_profile_dict[d][r] = (
...
#------------------------------------------------------- ###### Verkehr #######----------------------------------------------------------------------------
    ...
    if region:
        ...
        demand = pd.DataFrame()
        for d in demands:
            demand[d] = (demand_profile_dict[d]['north'] +
                         demand_profile_dict[d]['east'] +
                         demand_profile_dict[d]['middle'] +
                         demand_profile_dict[d]['swest'])
        return demand