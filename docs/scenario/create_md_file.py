# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 16:39:29 2024

@author: rbala
"""

from mdutils.mdutils import MdUtils
from mdutils import Html
import os



def create_simulation_doc(permutation, scenario_num, hypothese, sim_remarks,csv):

    md_PATH = os.path.abspath(os.path.join(os.getcwd(), "docs", "scenario"))
    github_link = "https://github.com/in-RET/ZO.RRO2/blob/new-structure"
    img_PATH = github_link + "/figures/"+ str(permutation) +'/'+ scenario_num + '/'
    YEAR, model_ID = permutation.split("_")

           
    mdFile = MdUtils(file_name=  os.path.join(md_PATH, str(permutation) + '_' +scenario_num +'.md'))
    mdFile.new_line("---\n"+
                         "Model ID: " + str(permutation) + "_" + scenario_num + "\n"+ 
                         "Simulation Year: " + YEAR + "\n"+
                         "Variation: " + model_ID + "\n"+
                         "Scenario_num: " + scenario_num +"\n"+
                         "OEMOF.solph version: 0.5.2 \n"+
                         "Keywords: \n"+
                             "- wind \n"+
                             "- pv \n"+
                         "---"
                         )
    mdFile.new_paragraph("**MD-File info**: ")                     
    mdFile.new_line("This is a markdown file created using mdutils python package. This file gives an overview of the "
                         "scenario and the changes in the energy model." 
                         "\n**IMPORTANT:** This file is automatically generated at the end of the simulation. "
                         "Key points from the analyis of the simulation must be added manually.")
    


    mdFile.new_header(level=1, title= 'Simulation : ')

    mdFile.new_paragraph("**Hypothese**: ")
    mdFile.new_line(hypothese) 
    mdFile.new_paragraph("**Remarks/changes**: ")
    mdFile.new_line(sim_remarks)
    
    mdFile.new_header(2, "Plots: ")
    
    image_text = "Leistung_Erneuerbare"
    path = img_PATH + scenario_num +'_' + image_text +'.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")   
    mdFile.new_paragraph(Html.image(path=path, size='400'))
    mdFile.new_line()

    image_text = "PtX-Technologien"
    path = img_PATH + scenario_num +'_' + image_text +'.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='400'))
    mdFile.new_line()

    image_text = "Speicherkapazitäten"
    path = img_PATH + scenario_num +'_' + image_text +'.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='400'))
    mdFile.new_line()

    image_text = "electricity_loadprofile"
    path = img_PATH + image_text+'_Heatmap.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='900x600'))
    mdFile.new_line()
    
    image_text = "gas_loadprofile"
    path = img_PATH + image_text+'_Heatmap.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='900x600'))
    mdFile.new_line()
    
    image_text = "oil_loadprofile"
    path = img_PATH + image_text+'_Heatmap.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='900x600'))
    mdFile.new_line()
    
    image_text = "dist_heating_loadprofile"
    path = img_PATH + image_text+'_Heatmap.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='900x600'))
    mdFile.new_line()
    
    image_text = "PV_Openfield"
    path = img_PATH + image_text+'_Heatmap.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='900x600'))
    mdFile.new_line()
    
    image_text = "Wind"
    path = img_PATH + image_text+'_Heatmap.png'+'?raw=true'
    mdFile.new_header(3, image_text)
    mdFile.new_line("["+image_text+"]("+path+")")  
    mdFile.new_paragraph(Html.image(path=path, size='900x600'))
    mdFile.new_line()

    

    return mdFile.create_md_file()